...
static esp_err_t click_post_handler(httpd_req_t *req)
{
    ...
    if (json)
   {
        ...
        if (received_message)
        {
            ...
            ESP_LOGI("Testing", "%s", received_message->valuestring);
            ...
            ESP_LOGI(TAG,"Data from webserver 1 %s ", message);
            ESP_LOGI(TAG, "NVS_DATA %s", message);
            ESP_LOGI(TAG,"Data from webserver 2 %s ", message);
            message_obj.str = message;
            ESP_LOGI(TAG,"message_obj %s ", message_obj.str);
